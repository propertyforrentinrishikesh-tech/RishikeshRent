...
export async function GET(request) {
    try {
        ...
        const properties = await PropertyDetails.find(query)
            .select('propertyName propertyType locationType subLocationType galiType mainImage galleryImages video contactAddress brokerName ownerName contactNumbers rentPrice maxRentPrice highlights isActive isTrending propertyFor propertyNameSlug')
            .sort({ createdAt: -1 })
            .lean();
