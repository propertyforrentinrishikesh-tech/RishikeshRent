...
export async function GET(request) {
    try {
        ...
        if (searchParams.get('propertyType')) {
            ...
            filter.propertyType = { $regex: new RegExp(`^\\s*${pType.replace(/-/g, '\\s+')}\\s*$`, 'i') };
        }
        if (searchParams.get('locationType')) {
            const lType = searchParams.get('locationType');
            filter.locationType = { $regex: new RegExp(`^\\s*${lType.replace(/-/g, '\\s+')}\\s*$`, 'i') };
        }
        if (searchParams.get('subLocationType')) {
            const slType = searchParams.get('subLocationType');
            filter.subLocationType = { $regex: new RegExp(`^\\s*${slType.replace(/-/g, '\\s+')}\\s*$`, 'i') };
        }
        if (searchParams.get('propertyFor')) {
            const pFor = searchParams.get('propertyFor');
            filter.propertyFor = { $regex: new RegExp(`^\\s*${pFor.replace(/-/g, '\\s+')}\\s*$`, 'i') };
        }
        ...
        if (searchParams.get('showOnFront') !== null && searchParams.get('showOnFront') !== undefined && searchParams.get('showOnFront') !== '') {
            filter.showOnFront = searchParams.get('showOnFront') === 'true';
        }
        // Rent range filter
        ...
        const total = await PropertyDetails.countDocuments(filter);
