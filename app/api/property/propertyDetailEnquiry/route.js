...
import PropertyDetailEnquiry from "@/models/Property/PropertyDetailEnquiry";
...
export async function GET(request) {
    try {
        ...
        const total = await PropertyDetailEnquiry.countDocuments(filter);
        const enquiries = await PropertyDetailEnquiry.find(filter)
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit)
        ...
        console.error("PropertyDetailEnquiry GET error:", error);
...
export async function POST(request) {
    try {
        ...
        console.error("PropertyDetailEnquiry POST error:", error);
...
export async function PUT(request) {
    try {
        ...
        const body = await request.json();
        const updated = await PropertyDetailEnquiry.findByIdAndUpdate(id, body, { new: true });
        if (!updated) return NextResponse.json({ success: false, message: "Enquiry not found" }, { status: 404 });

        return NextResponse.json({ success: true, data: updated });
    } catch (error) {
        console.error("PropertyDetailEnquiry PUT error:", error);
...
export async function DELETE(request) {
    try {
        ...
        const deleted = await PropertyDetailEnquiry.findByIdAndDelete(id);
        if (!deleted) return NextResponse.json({ success: false, message: "Enquiry not found" }, { status: 404 });

        return NextResponse.json({ success: true, message: "Enquiry deleted" });
    } catch (error) {
        console.error("PropertyDetailEnquiry DELETE error:", error);
