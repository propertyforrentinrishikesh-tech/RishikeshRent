...
import HotelSubLocation from "@/models/Hotels/HotelSubLocation";


export async function GET() {
    await connectDB();
    try {
        const banners = await HotelSubLocation.find()
...
export async function POST(req) {
    ...
    try {
        ...
        const newBanner = new HotelSubLocation({ locationType,subLocationType });
...
export async function PATCH(req) {
    ...
    try {
        ...
        const updatedBanner = await HotelSubLocation.findByIdAndUpdate(id, { locationType,subLocationType,isActive }, { new: true });
...
export async function DELETE(req) {
    ...
    try {
        ...
        const banner = await HotelSubLocation.findById(id);
        if (!banner) {
            return NextResponse.json({ error: "SubLocation not found" }, { status: 404 });
        }
        // Delete banner from database
        await HotelSubLocation.findByIdAndDelete(id);
