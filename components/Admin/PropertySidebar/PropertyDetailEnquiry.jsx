...
// ─── Main Component ────────────────────────────────────────────────────────────
const PropertyDetailEnquiry = ({ locationType = [] }) => {
  ...
  const fetchEnquiries = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/property/propertyDetailEnquiry?${params.toString()}`);
  ...
  const updateStatus = async (enquiry, newStatus) => {
    ...
    try {
      const res = await fetch(`/api/property/propertyDetailEnquiry?id=${enquiry._id}`, {
        method: "PUT",
  ...
  const saveNote = async () => {
    ...
    setSavingNote(true);
    try {
      const res = await fetch(`/api/property/propertyDetailEnquiry?id=${selectedEnquiry._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/property/propertyDetailEnquiry?id=${enquiryToDelete._id}`, { method: "DELETE" });
      const data = await res.json();
      if (data.success) {
...
export default PropertyDetailEnquiry;