...
const WeekendPrice = ({ propertyData }) => {
    // Form States
    ...
    // Weekend Day Selection (All checked by default)
    ...
    const handleUpdate = async () => {
        ...
        // Check if at least one weekend day is selected
        if (!selectedDays.friday && !selectedDays.saturday && !selectedDays.sunday) {
            toast.error('Please select at least one weekend day (Friday, Saturday, or Sunday)')
            return
        }
        ...
        try {
            const pricingData = {
                ...
                endDate,
                pricingType: 'weekend',
                weekendDays: selectedDays, // Pass selected weekend days
                epPlan: {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 space-y-6">
            ...
            {/* Weekend Day Selection - Only show if dates and room are selected */}
            {startDate && endDate && selectedRoom && (
                <Card className="border-2 border-purple-200 bg-purple-50">
                    <CardContent className="p-6">
                        <div className="space-y-4">
                            <div>
                                <h3 className="font-bold text-lg text-purple-900 mb-2">Select Specific Day</h3>
                                <p className="text-sm text-purple-700">
                                    Weekend Escape Rates (Friday & Saturday nights) - "Dynamic Pricing," where rates increase based on higher demand.
                                </p>
                            </div>

                            {/* Weekend Day Checkboxes */}
                            <div className="grid grid-cols-3 gap-4">
                                <div className="flex items-center gap-3 bg-white p-4 rounded-lg border-2 border-purple-200 hover:border-purple-400 transition-colors">
                                    <input
                                        type="checkbox"
                                        id="friday"
                                        ...
                                        onChange={(e) => {
                                            ...
                                            // Prevent unchecking if it's the last checked day
                                            if (!newValue && !selectedDays.saturday && !selectedDays.sunday) {
                                                toast.error('At least one weekend day must be selected');
                                                return;
                                            }
                                            setSelectedDays({ ...selectedDays, friday: newValue });
                                        }}
                                        className="w-5 h-5 text-purple-600 border-gray-300 rounded focus:ring-purple-500"
                                    />
                                    <label htmlFor="friday" className="text-base font-semibold text-gray-900 cursor-pointer">
                                        Friday
                                    </label>
                                </div>
                                ...
                                <div className="flex items-center gap-3 bg-white p-4 rounded-lg border-2 border-purple-200 hover:border-purple-400 transition-colors">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            const newValue = e.target.checked;
                                            // Prevent unchecking if it's the last checked day
                                            if (!newValue && !selectedDays.friday && !selectedDays.sunday) {
                                                toast.error('At least one weekend day must be selected');
                                                return;
                                            }
                                            setSelectedDays({ ...selectedDays, saturday: newValue });
                                        }}
                                        className="w-5 h-5 text-purple-600 border-gray-300 rounded focus:ring-purple-500"
                                    />
                                    <label htmlFor="saturday" className="text-base font-semibold text-gray-900 cursor-pointer">
                                        Saturday
                                    </label>
                                </div>
                                ...
                                <div className="flex items-center gap-3 bg-white p-4 rounded-lg border-2 border-purple-200 hover:border-purple-400 transition-colors">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            const newValue = e.target.checked;
                                            // Prevent unchecking if it's the last checked day
                                            if (!newValue && !selectedDays.friday && !selectedDays.saturday) {
                                                toast.error('At least one weekend day must be selected');
                                                return;
                                            }
                                            setSelectedDays({ ...selectedDays, sunday: newValue });
                                        }}
                                        className="w-5 h-5 text-purple-600 border-gray-300 rounded focus:ring-purple-500"
                                    />
                                    <label htmlFor="sunday" className="text-base font-semibold text-gray-900 cursor-pointer">
                                        Sunday
                                    </label>
                                </div>
                            </div>
                            ...
                            {/* Selected Days Info */}
                            {(selectedDays.friday || selectedDays.saturday || selectedDays.sunday) && (
            ...
            {startDate && endDate && selectedRoom && (selectedDays.friday || selectedDays.saturday || selectedDays.sunday) && (
                <>
                    {/* Date Range Info */}
                    <Card className="border-2 border-purple-200 bg-purple-50">
                        <CardContent className="p-6">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <h3 className="font-bold text-lg text-purple-900">Weekend Pricing Update</h3>
                                    ...
                                    <p className="text-purple-700 text-xs mt-2">
                                        Only updating: <strong>{[
                                            selectedDays.friday && 'Fridays',
                                            selectedDays.saturday && 'Saturdays',
                                            selectedDays.sunday && 'Sundays'
                                        ].filter(Boolean).join(', ')}</strong> within this range.
                                    </p>
                                </div>
                            </div>
                        </CardContent>
                ...
                <Card className="border-2 border-purple-200 bg-purple-50">
                    <CardContent className="p-6 text-center">
                        <p className="text-purple-900 font-semibold">
                            👆 Please select start date, end date, and room category to set weekend pricing
                        </p>
                    </CardContent>
                </Card>
            ) : null}
