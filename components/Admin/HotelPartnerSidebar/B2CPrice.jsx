...
const B2CPrice = ({ propertyData }) => {
    ...
    // Fetch current pricing when date and room are selected
    useEffect(() => {
        if (selectedDate && selectedRoom && propertyData?._id) {
            fetchCurrentPricing()
        }
    ...
    // Fetch all pricing data when date is selected
    useEffect(() => {
        if (selectedDate && propertyData?._id) {
            fetchAllPricingForDate()
        }
    }, [selectedDate])
    ...
    const fetchCurrentPricing = async () => {
        ...
        try {
            const response = await fetch(
                `/api/room-pricing?propertyId=${propertyData._id}&roomType=${selectedRoom}&date=${selectedDate}`
            )
    ...
    const fetchAllPricingForDate = async () => {
        if (!selectedDate || !propertyData?._id) return;

        setLoadingSummary(true)
        try {
            const response = await fetch(
                `/api/room-pricing?propertyId=${propertyData._id}&date=${selectedDate}`
            )
            const data = await response.json()
    ...
    const handleUpdate = async () => {
        ...
        if (!selectedDate || !selectedRoom) {
            toast.error('Please select date and room category')
            return
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                fetchCurrentPricing() // Refresh current pricing
                fetchAllPricingForDate() // Refresh summary table
            } else {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 space-y-6">
            ...
            {/* Show pricing section only if date and room are selected */}
            {selectedDate && selectedRoom && (
                <>
                    {/* Loading State */}
                    {loading && (
                        <Card>
                            <CardContent className="p-8 text-center">
                                ...
                                <p className="mt-2 text-gray-600">Loading current pricing...</p>
                            </CardContent>
                        </Card>
                    )}
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                        <h3 className="font-bold text-lg text-green-900 mb-3">
                                            Current Pricing for {new Date(selectedDate).toLocaleDateString('en-IN', {
                                                weekday: 'long',
                                                year: 'numeric',
                                                month: 'long',
                                                day: 'numeric'
                                        ...
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            <div>
                                                <p className="font-semibold text-gray-700">EP Plan:</p>
                                                <p className="text-gray-900">
                                                    1P: {formatCurrency(currentPricing.epPlan.person1)} |
                                                    2P: {formatCurrency(currentPricing.epPlan.person2)}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="font-semibold text-gray-700">CP Plan:</p>
                                                <p className="text-gray-900">
                                                    1P: {formatCurrency(currentPricing.cpPlan.person1)} |
                                                    2P: {formatCurrency(currentPricing.cpPlan.person2)}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="font-semibold text-gray-700">MAP Plan:</p>
                                                <p className="text-gray-900">
                                                    1P: {formatCurrency(currentPricing.mapPlan.person1)} |
                                                    2P: {formatCurrency(currentPricing.mapPlan.person2)}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="font-semibold text-gray-700">AP Plan:</p>
                                                <p className="text-gray-900">
                                                    1P: {formatCurrency(currentPricing.apPlan.person1)} |
                                                    2P: {formatCurrency(currentPricing.apPlan.person2)}
                                                </p>
                                            </div>
                                        </div>
                                        <p className="text-xs text-gray-600 mt-3">
                                            Last updated: {new Date(currentPricing.updatedAt).toLocaleString('en-IN')}
                                        </p>
                                    </div>
                                </div>
                            </CardContent>
                ...
                <Card className="border-2 border-blue-200 bg-blue-50">
                    <CardContent className="p-6 text-center">
                        <p className="text-blue-900 font-semibold">
                            👆 Please select a date and room category to view and update pricing
                        </p>
                    </CardContent>
                </Card>
            ) : null}
            ...
            {selectedDate && allPricingData.length > 0 && (
                <Card className="mt-8">
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-2xl font-bold text-gray-900">
                                Update Price Chart - {new Date(selectedDate).toLocaleDateString('en-IN', {
                                    day: '2-digit',
                                    month: '2-digit',
                                    year: 'numeric'
                                })}
