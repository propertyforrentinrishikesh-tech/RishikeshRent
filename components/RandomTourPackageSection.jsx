...
const RandomTourPackageSection = () => {
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      ...
      const generateSlug = (text) => text?.toString().trim().toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, '').replace(/--+/g, '-');
      const finalSlug = selectedBookingProperty.propertyNameSlug || generateSlug(selectedBookingProperty.propertyName || "");
      const propertyUrl = `${window.location.origin}/properties/${selectedBookingProperty.locationType?.toLowerCase().replace(/\s+/g, '-') || ''}/${finalSlug}`;
        ...
        <div className="flex flex-col mb-8">
          <div className="flex justify-between items-end border-b border-gray-200 pb-2">
            <div className="flex flex-col">
              <span className="text-xs uppercase tracking-widest text-gray-900 mb-1">Experience the Unrivaled: Our Deluxe Room Offer</span>
              <h2 className="text-2xl md:text-3xl font-semibold text-gray-900">A Quiet Escape For Modern Travelers Like You</h2>
            </div>
            <Link href="/properties" className="text-sm font-semibold text-gray-800 hover:underline mb-1 hidden md:block">
              View More
            </Link>
          </div>
          <div className="mt-2 text-sm text-gray-700 tracking-wide">
